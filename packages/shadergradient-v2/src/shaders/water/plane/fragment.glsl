// Water Fragment Shader for Plane geometry
...
void main() {
  ...
  if (caustic > 0.0) {
    float causticVal = causticPattern(vUv, time);
    color = mix(color, color3, causticVal * caustic);
  }
  
  // Add edge glow
  if (edges > 0.0) {
    float edge = edgeGlow(vNormal, viewDir);
  ...
  if (highlights > 0.0) {
    ...
    color = mix(color, color3, highlight * highlights);
  }
