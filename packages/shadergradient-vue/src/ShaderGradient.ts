...
export const ShaderGradient = defineComponent({
  ...
  setup(passedProps, { emit }) {
    ...
    watchEffect(() => {
      if (context.renderer.isInitialized.value)
        activeRenderer = context.renderer.instance as THREE.WebGLRenderer
    })
    onScopeDispose(() => {
    ...
    watch(
      [context.camera.activeCamera, context.renderer.isInitialized],
      ([camera, isInitialized], _previous, cleanup) => {
        if (!isInitialized || !(camera instanceof THREE.PerspectiveCamera)) return
    ...
    // Tres drives the frame through this callback; the composer keeps its own clock.
    loop.render((notifySuccess) => {
      const camera = context.camera.activeCamera.value
      if (composer) composer.render()
      else if (camera)
        (context.renderer.instance as THREE.WebGLRenderer).render(
          context.scene.value,
          camera
        )
      notifySuccess()
