...
export const Line = React.forwardRef<Line2, LineProps>(function Line(
  ...
  const [lineMaterial]: any = React.useState(() => new LineMaterial())
  const lineGeom: any = React.useMemo(() => {
    const geom = new LineGeometry()
    const pValues: any = points.map((p) => {
    ...
    if (vertexColors) {
      const cValues: any = vertexColors.map((c) =>
