// Impasto Sphere Fragment Shader - oil paint dragged by a coarse brush:
...
void main() {
  ...
  // Oblique projection of the sphere surface keeps the strokes wrapping around it
  vec2 p = vec2(vPos.x + 0.65 * vPos.z, vPos.y - 0.35 * vPos.z) * 2.6 + vec2(2.7, 1.3);
