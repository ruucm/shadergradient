// Silk Sphere Vertex Shader - Apple-style flowing silk
...
vec3 silkDisplace(vec3 dir, float t, out vec3 field) {
  field = silkField(dir * 1.7, t);
  return dir * (1.0 + field.x * 0.55 * uNoiseStrength * uLoadingTime);
}
...
void main() {
  ...
  // walk a small step along the sphere in two tangent directions, displace all
  // three points and rebuild the normal from the displaced triangle
  vec3 dir = normalize(position);
  vec3 up = abs(dir.y) < 0.99 ? vec3(0.0, 1.0, 0.0) : vec3(1.0, 0.0, 0.0);
  vec3 silkT = normalize(cross(up, dir));
  vec3 silkB = cross(dir, silkT);
  float eps = 0.02;

  vec3 field;
  vec3 fieldT;
  vec3 fieldB;
  vec3 pos = silkDisplace(dir, t, field);
  vec3 posT = silkDisplace(normalize(dir + silkT * eps), t, fieldT);
  vec3 posB = silkDisplace(normalize(dir + silkB * eps), t, fieldB);

  vec3 silkNormal = normalize(cross(posT - pos, posB - pos));

  // gentle twist, same controls as the defaults sphere; when looping the
  // phase runs on a circle so it lands back where it started
  float twistT = uLoop > 0.5
    ? cos((uTime / uLoopDuration) * 6.28318530718) * 5.0 * uSpeed
    : t;
  float angle = sin(uv.y * uFrequency + twistT) * uAmplitude;
  pos = rotateY(pos, angle);
  silkNormal = rotateY(silkNormal, angle);
