// Transmission-inspired plane vertex shader
...
void main() {
  ...
  vec3 noisePos = vec3(position.x * uNoiseScale, position.y * uNoiseScale, time);
  float noise = snoise(noisePos) * uDistortion;

  float wave = sin((uv.x + uv.y + time) * 6.28318) * 0.35 * uDistortion;
  float distortion = noise + wave;
  vDistortion = distortion;
  
  transformed += normal * distortion * 0.6;
  transformed.xz += normal.xz * distortion * 0.2;
  ...
  float ior = 1.1 + uRefraction * 0.6 + distortion * 0.15;
  vRefractDir = refract(-viewVector, worldNormal, 1.0 / max(ior, 1.01));
  
  vIridescence = dot(worldNormal, vec3(0.0, 1.0, 0.0)) + distortion * 0.5;
